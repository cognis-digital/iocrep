...
[![PyPI](https://img.shields.io/pypi/v/cognis-iocrep.svg)](https://pypi.org/project/cognis-iocrep/)
[![CI](https://github.com/cognis-digital/iocrep/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/iocrep/actions)
...
IOCREP exists for one job — score iocs against offline reputation/allow lists with explainable verdicts — and does it without a SaaS bill or heavyweight setup.
...
```bash
iocrep --version
iocrep scan .                      # scan the current project
iocrep scan . --format json
iocrep scan . --fail-on high       # non-zero exit for CI gates
iocrep mcp                         # expose as an MCP server (Cognis.Studio / Claude Desktop / Cursor)
```
...
Built in the spirit of **-**, re-framed for the Cognis approach: single-purpose, self-hostable,
MCP-native, and unified with the rest of the Suite. Missing a credit? Open a PR.

## How it fits the Cognis Neural Suite

`iocrep` is one of the **100+ tools** in the [Cognis Neural Suite](https://github.com/cognis-digital).

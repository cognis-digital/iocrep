"""Native cognis-connect emit for iocrep — forward findings to any platform.

Maps iocrep's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    iocrep ... --format json | iocrep-emit --to stix
    iocrep-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        if 'title' not in out:
            out['title'] = rec.get('indicator', '') + ' - ' + rec.get('verdict', '')
        if 'severity' not in out:
            out['severity'] = rec.get('verdict', '').lower()
        if 'type' not in out:
            out['type'] = rec.get('kind', '')
        if 'description' not in out:
            out['description'] = rec.get('reason', '')
        if 'tags' not in out:
            out['tags'] = []
            if rec.get('verdict') == 'allow':
                out['tags'].append('allowlisted')
            if rec.get('verdict') in ['low', 'medium', 'high', 'critical']:
                out['tags'].append('finding')
        if 'ipv4' not in out:
            out['ipv4'] = rec.get('indicator', '')
